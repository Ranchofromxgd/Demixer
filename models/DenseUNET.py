...
class DenseUnet(nn.Module):
    ...
    if (Config.layer_numbers_unet == 5):
        def __init__(self, n_channels, n_classes, bilinear=True):
            ...
            # self.up1 = Up(self.getCh(6,6), 1024, bilinear = True,TIF_in=0)
            self.up2 = Up(self.getCh(5,6), 512, bilinear = True,TIF_in = 64)
            self.up3 = Up(self.getCh(4)+512, 256, bilinear = True,TIF_in = 128)
            self.up4 = Up(self.getCh(3)+256, 128, bilinear = True,TIF_in = 256)
            self.up5 = Up(self.getCh(2)+128, 64, bilinear = True,TIF_in = 512)
            self.up6 = Up(self.getCh(1)+64, 64, bilinear = True,TIF_in = 1025)
...
class DenseConv(nn.Module):
    def __init__(self, in_channels, out_channels,TIF_in = 2048):
        ...
        self.TIF_in = TIF_in
        if(TIF_in != 0):self.tif = TIFblock(TIF_in,out_channels)
...
if __name__ == "__main__":
    ...
    model2 = DenseUnet(n_channels = 2,n_classes = 2)
    # print(get_parameter_number(model))
    print(model2)
    print(get_parameter_number(model2))
