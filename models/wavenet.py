...
class ResidualBlock(nn.Module):
    def __init__(self, res_channels, skip_channels, dilation):
        ...
        self.filter_conv = DilatedConv1d(in_channels=res_channels, out_channels=res_channels, dilation=dilation,padding=dilation)
        self.gate_conv = DilatedConv1d(in_channels=res_channels, out_channels=res_channels, dilation=dilation,padding=dilation)
...
class WaveNet(nn.Module):
    def __init__(self, in_depth=256, res_channels=32, skip_channels=512, dilation_depth=10, n_repeat=3):
        ...
        self.pre = nn.Embedding(in_depth, res_channels) # 256,32
    ...
    def forward(self, inputs):
        ...
        outputs = sum([s[:, :, -outputs.size(2):] for s in skip_connections]) # Only take the tail part
    ...
    def train(self,optimizer,song,vocal):
        output = self.forward(Variable(song))
        # loss
        loss = torch.nn.L1Loss(output,vocal)
        # 向后传播
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
