...
def train( # Frequency domain
    ...
    for track_i in range(Config.channels):
        ...
        if(track_i == Config.channels-1):
            ...
            if('l4' in Config.loss_component or 'l5' in Config.loss_component or 'l6' in Config.loss_component):
            ...
            if('l2' in Config.loss_component):lossVal = 0.001*loss(output_mixed,target_mixed)
            if('l3' in Config.loss_component):lossVal += 0.001*loss(output_vocal,target_vocal)
...
if(not Config.start_point == 0):
    model = torch.load("/home/work_nfs/hhliu/workspace/github/wavenet-aslp/saved_models/"+Config.trail_name+"/model"+str(Config.start_point)+".pkl",
