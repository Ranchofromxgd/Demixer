...
name = "2020_1_13_Demixer_spleeter_l1_l2_l3_lr0001_bs1_fl2_ss60000_8lnu3mu0.5sig0.2low0.49hig0.5"
...
class SpleeterUtil:
    ...
    def evaluate(self, save_wav=True, save_json=True):
        ...
        for each in pth:
            ...
            if (save_wav == True):
                if (not os.path.exists(
                        Config.project_root + "outputs/musdb_test/" + Config.trail_name + str(self.model.cnt) + "/")):
                    print("mkdir: " + Config.project_root + "outputs/musdb_test/" + Config.trail_name + str(
                        self.model.cnt) + "/")
                    os.mkdir(
                        Config.project_root + "outputs/musdb_test/" + Config.trail_name + str(self.model.cnt) + "/")
                self.wh.save_wave((background[:background_min_length]).astype(np.int16),
                                  Config.project_root + "outputs/musdb_test/" + Config.trail_name + str(
                                      self.model.cnt) + "/background_" + each + ".wav",
                                  channels=1)
                ...
                self.wh.save_wave((vocal[:vocal_min_length]).astype(np.int16),
                                  Config.project_root + "outputs/musdb_test/" + Config.trail_name + str(
                                      self.model.cnt) + "/vocal_" + each + ".wav",
                                  channels=1)
        ...
        if (save_json == True):
            write_json(performance, Config.project_root + "outputs/musdb_test/" + Config.trail_name + str(
                self.model.cnt) + "/result_" + Config.trail_name + str(self.model.cnt) + ".json")
        print("Result:")
