...
class Crawler():
    ...
    def search(self, search_content, search_type, limit=9):
        ...
        params = {'s': search_content, 'type': search_type, 'offset': offset, 'sub': 'false', 'limit': limit}
...
class Netease():
    ...
    def download_song_by_search(self, song_name, song_num,limit):
        """
        根据歌曲名进行搜索
        :params song_name: 歌曲名字
        :params song_num: 下载的歌曲数
        ...
        song = None
        try:
            song = self.crawler.search_song(song_name, song_num, self.quiet,limit=limit)
        except:
...
if __name__ == '__main__':
    from wave_util import WaveHandler
    import time
    import numpy as np
    wh = WaveHandler()
    timeout = 60
    ...
    idlist = wh.load_pickle("idlist.pkl")
    songsList = wh.load_pickle("songlist.pkl")
    for _ in range(1000):
        songs = netease.download_song_by_search("(INSTRUMENTAL VERSION )", 1000,limit=10)
        for each in songs:
            if(not each['id'] in idlist):
                idlist.append(each['id'])
                songsList.append(each)
        offset += 100
        wh.save_pickle(idlist,"idlist.pkl")
        wh.save_pickle(songsList,"songlist.pkl")
        sleepTime = np.random.randint(1,10)
        time.sleep(sleepTime)
        print("Current Length: ",len(idlist))
    # 如果music列表存在, 那么开始下载
