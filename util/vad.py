...
class VoiceActivityDetector():
    ...
    def __init__(self, wave_input_filename = None,rate = None,data = None,channels = None,silence_min_length = 1):
        if(wave_input_filename != None):
            self._read_wav(wave_input_filename)._convert_to_mono()
        else:
            self._feed_data(rate,data,channels)
        self.sml = silence_min_length
    ...
    def _feed_data(self,rate,data,channels):
        self.rate,self.data,self.channels = rate,data,channels
        return self
    ...
    def select_speech_regions(self):
        """ Performs speech detection and plot original signal and speech regions.
        """
        detected_windows = self.detect_speech()
        data_mask = np.ones(len(self.data))
        start,end = None,None
        vocal = True
        for i in range(detected_windows.shape[0]):
            if(abs(detected_windows[i,1]) <= 1e-3 and vocal == True):
                vocal = False
                start = detected_windows[i,0]
                continue
            elif(abs(detected_windows[i,1]-1) <= 1e-3 and vocal == False):
                end = detected_windows[i,0]
                vocal = True
                start,end = int(start),int(end)
                if(((end-start)/self.rate)>self.sml):
                    data_mask[start:end] = np.zeros(end-start)
...
if __name__ == "__main__":
    filename = 'vocal_train_0.wav'
    v = VoiceActivityDetector(filename)
    v.select_speech_regions()