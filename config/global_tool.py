...
class GlobalTool:
    ...
    if(Config.split_band):
        if (Config.use_gpu):
            qmf = PQMF(subband_number, 64).cuda(Config.device)
        else:
            qmf = PQMF(subband_number, 64)

    @classmethod
    def refresh_subband(self,subband_number):
        if (Config.split_band):
            if (Config.use_gpu):
                GlobalTool.qmf = PQMF(subband_number, 64).cuda(Config.device)
            else:
                GlobalTool.qmf = PQMF(subband_number, 64)
